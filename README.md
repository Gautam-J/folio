...
Folio server
   ...
   │     date/time and progress just read the clock)
...
- `internal/widget` — the `Widget` interface and its implementations (`WeatherWidget`, `DateTimeWidget`, `ProgressWidget`); each owns its own data fetch and template
...
Current scope is a multi-widget dashboard (weather, date/time, and week/month/year progress bars), LAN-only, with a shared-token auth model. GitHub stats, daily quotes, mental models, word of the day, and RSS tech news widgets are potential future directions — see `docs/superpowers/specs/2026-09-12-dashboard-mashup-design.md`'s "Future work" section — not yet built.
