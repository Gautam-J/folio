...
GET /api/display (access-token, png-width, png-height headers)
  → handlers.Server.HandleDisplay
      → build []widget.Widget{weatherWidget, dateTimeWidget, quoteWidget, progressWidget, onThisDayWidget, githubStatsWidget}  — constructed once at startup in cmd/main.go
...
- **`internal/github`** — GitHub REST (repo star totals, no token needed) + GraphQL (`contributionsCollection` — needs `github_token` even for public data) client, same cache-last-good-on-failure shape as the others.
- **`internal/render`** — `RenderDashboard` calls each widget's `Render` to get an HTML fragment, slots them positionally into six fixed regions (widgets[0] → Main, widgets[1] → Corner, widgets[2] → Strip, widgets[3] → Bottom, widgets[4] → Aside, widgets[5] → AsideTop), executes `templates/dashboard.html` as a Go template with those fragments, writes it to `generated/render.html`, then drives headless Chromium via `chromedp` (pinned to v0.10.0 — see Gotchas) to screenshot it at the requested `png-width`/`png-height` into `generated/current.png`.
- **`internal/config`** — loads and validates `config.yaml` (real file is gitignored; copy from `config.example.yaml`). Fails fast if `access_token`, `port`, or `refresh_rate_seconds` are missing/invalid. `github_username`/`github_token` are optional — left empty, the GitHub stats widget's fetch just fails and renders blank, same as any other widget error.
...
- `.view` is a flex column with `.dashboard-main` (Main+Corner+Aside+AsideTop) and `.dashboard-footer` (Strip+Bottom) as its two items, specifically so they stack without overlapping regardless of how tall the widgets' content is. `.dashboard-main` needs `flex: 1 1 auto` for this to work — without a definite height there, a widget's `.layout { height: 100% }` resolves against an indefinite parent and collapses to zero.
- Inside `.dashboard-main`, Main and Aside sit in a `.dashboard-main-row` flex row. TRMNL's `.column{width:0;flex:1}` and `.columns{width:100%}` need a *definite* width/height on every ancestor up to `.layout` — a flex item sized by `flex-basis:auto` (content-based) never provides one, and the whole chain silently collapses to 0×0 (no error, just missing content). Give each row item a definite size instead: a percentage `flex-basis` for width, `align-items: stretch` on the row for height.
- TRMNL's `.label` class is `display: inline-flex`, not block. `text-align` on an ancestor has no effect on it — flex layout only respects `justify-content`, which `.label` doesn't set, so content defaults to `flex-start` (left). This looks harmless when a `.label` is alone in a box exactly as wide as its own text (left- and right-aligned are visually identical then), but breaks visibly the moment a wider sibling stretches that box — as happened when the GitHub stats widget's longer line was added under the corner date. Fix: force `display: block` on the specific class so `text-align` applies (see `.datetime-date`, `.label.github-stats-primary`).
- Comparing a text element's vertical position against another (cross-widget alignment) via `getBoundingClientRect()` can be misleading when the two elements sit in different structures (e.g. a flex row with an icon vs. a plain label) — the row's own box top is set by whichever child is tallest (often the icon), not the text baseline. Measure the actual glyph position instead: `document.createRange().selectNodeContents(el)` + `getClientRects()[0].top`, or fall back to a raw pixel scan of the rendered PNG.
...
Current scope is a multi-widget dashboard (weather, date/time, quote, progress, on-this-day, GitHub stats) via the `internal/widget` framework, LAN-only, single shared-token auth. `docs/superpowers/specs/2026-09-12-dashboard-mashup-design.md` and the matching plan in `docs/superpowers/plans/` are the design/implementation docs this framework was built from; its "Future work" section listed daily quotes and a GitHub stats widget among what was deferred — both are now built (quotes as random, not daily, per the user's direction). Still deferred: an RSS tech news widget, a ZenQuotes-backed inspirational image widget (`zenquotes.io/api/image`), and an xkcd comic widget (`https://xkcd.com/<random_number>/info.0.json`, returns an image URL). The dotted sidebar next to the main temperature, a dotted/halftone background (on the progress-bar widget rather than the quote), a "feels like" apparent temperature line below the main temperature, an "On this day" widget (`today.zenquotes.io/api`, beside the weather widget), and a GitHub stats widget (yearly contributions, total stars, commits/PRs/issues, left-aligned in the same column as on-this-day) are all built. The on-this-day event text isn't truncated yet — an unusually long entry combined with a wrapped quote can overflow into the footer; ~150 characters is the safe budget before truncation would be needed at the 1648×1236 render size.
