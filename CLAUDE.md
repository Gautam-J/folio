...
- **`internal/render`** — `RenderDashboard` calls each widget's `Render` to get an HTML fragment, slots them positionally into five fixed regions (widgets[0] → Main, widgets[1] → Corner, widgets[2] → Strip, widgets[3] → Bottom, widgets[4] → Aside), executes `templates/dashboard.html` as a Go template with those fragments, writes it to `generated/render.html`, then drives headless Chromium via `chromedp` (pinned to v0.10.0 — see Gotchas) to screenshot it at the requested `png-width`/`png-height` into `generated/current.png`.
...
Current scope is a multi-widget dashboard (weather, date/time, quote, progress, on-this-day) via the `internal/widget` framework, LAN-only, single shared-token auth. `docs/superpowers/specs/2026-09-12-dashboard-mashup-design.md` and the matching plan in `docs/superpowers/plans/` are the design/implementation docs this framework was built from; its "Future work" section listed daily quotes among what was deferred — that's now built (as a random, not daily, quote per the user's direction). Still deferred: GitHub stats and RSS tech news widgets, plus a ZenQuotes-backed inspirational image widget (`zenquotes.io/api/image`) and an xkcd comic widget (`https://xkcd.com/<random_number>/info.0.json`, returns an image URL). The dotted sidebar next to the main temperature, a dotted/halftone background (on the progress-bar widget rather than the quote), a "feels like" apparent temperature line below the main temperature, and an "On this day" widget (`today.zenquotes.io/api`, beside the weather widget) are all built. The on-this-day event text isn't truncated yet — an unusually long entry combined with a wrapped quote can overflow into the footer; ~150 characters is the safe budget before truncation would be needed at the 1648×1236 render size.
