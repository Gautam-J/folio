...
Current scope is a multi-widget dashboard (weather, date/time, quote, progress) via the `internal/widget` framework, LAN-only, single shared-token auth. `docs/superpowers/specs/2026-09-12-dashboard-mashup-design.md` and the matching plan in `docs/superpowers/plans/` are the design/implementation docs this framework was built from; its "Future work" section listed daily quotes among what was deferred — that's now built (as a random, not daily, quote per the user's direction). Still deferred: GitHub stats and RSS tech news widgets, plus two more ZenQuotes-backed widgets — "On this day" (`today.zenquotes.io/api`) and an inspirational image (`zenquotes.io/api/image`) — and an xkcd comic widget (`https://xkcd.com/<random_number>/info.0.json`, returns an image URL). Also deferred, a smaller polish item: a "feels like" apparent temperature line below the main temperature. (The dotted sidebar next to the main temperature and a dotted/halftone background are both built — the latter ended up on the progress-bar widget rather than the quote.)
